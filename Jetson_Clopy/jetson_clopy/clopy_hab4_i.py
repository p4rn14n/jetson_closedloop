...
freqs = [600 * (2**(1/4))**i for i in range(n_tones)]
...
for session, reward_threshold in sessions:
    ...
    sinsource = SineSource(austream, 600)
    sinsource.stop()
    #############
    ...
    while image2 is not None and nTrial <= total_trials:
        ...
        if runTrial:
            ...
            if deliverReward:
                ...
                # t2 = threading.Thread(target=blink_led_pymata, args=(ledReward, 0.3))
                # t2.start()
                lastRewTime = time.time()
                runTrial = False
        ...
        logFile.write(str(fps._numFrames) + '\t' +
                      ...
                      str(audio) + '\t' +
                      str(totRewards) + '\t' +
                      str(np.mean(runningImgQuROI[0])) + '\t' +
                      str(np.mean(runningImgQuROI[1])) + '\t' +
                    #   str(runningImgQuROI[0]) + '\t' +
                    #   str(runningImgQuROI[1]) + '\t' +
                      str(np.std(runningImgQuROI[0])) + '\t' +
                      str(np.std(runningImgQuROI[1])) + '\n')
                    #   str(touch) + '\n')
                    # str(totRewards) + '\t' +
